    ...
    <div className="border border-neutral-200 divide-y divide-neutral-100">
      {fields.map(({ key, value }) => (
        <div key={key} className="flex items-baseline gap-4 px-5 py-2.5">
          <span className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400 flex-shrink-0 w-20">
            {key}
          </span>
          <span className="text-xs text-neutral-700 font-mono">{value}</span>
        </div>
      ))}
      {table.note && (
