...
import { Search, Database, BarChart2, ChevronDown, ChevronUp, ExternalLink, CheckCircle } from "lucide-react";
...
const MOCK_ROWS = [
  ...
  ["Riau", "4.31"],
  ["Jambi", "4.67"],
  ["Sumatera Selatan", "4.09"],
  ["Bengkulu", "3.42"],
  ["Lampung", "4.23"],
  ["Kep. Bangka Belitung", "4.55"],
  ["Kepulauan Riau", "6.11"],
  ["DKI Jakarta", "6.53"],
  ["Jawa Barat", "7.44"],
  ["Jawa Tengah", "5.13"],
...
const CHART_TYPES = [
  { group: "Bar (Horizontal)", items: ["Bar Chart (1 seri)", "Grouped Bars (multi-seri)", "Stacked Bars", "Split Bars"] },
...
export function MinimalMono() {
  ...
  return (
    <div className="min-h-screen bg-white font-['Inter',sans-serif]" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="border border-neutral-200 p-5 space-y-4">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-neutral-500">Masukkan URL Data JSON BPS</h2>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              <button className="flex items-center gap-2 px-5 py-2 text-xs font-semibold bg-neutral-900 text-white hover:bg-neutral-700 transition-colors tracking-wide">
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {[
              { label: "FORMAT", value: "dynamic" },
              { label: "KOLOM", value: "2" },
              { label: "BARIS", value: "34" },
            ].map(({ label, value }) => (
          ...
          <div className="border border-neutral-200 p-5">
            ...
            <table className="w-full">
              <thead>
                <tr>
                  {MOCK_COLS.map((col) => <ColHeader key={col}>{col}</ColHeader>)}
                </tr>
              </thead>
              <tbody>
                {MOCK_ROWS.map((row, i) => (
            ...
            <p className="text-xs text-neutral-400 mt-3">Menampilkan 15 dari 34 baris · Klik header kolom untuk mengganti nama</p>
          </div>
          ...
          <div className="border border-neutral-200 p-5 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-semibold uppercase tracking-widest text-neutral-500">Buat Visualisasi di Datawrapper</h3>
            </div>
            ...
            {chartCreated ? (
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-widest text-neutral-500 mb-1.5">Deskripsi <span className="font-normal normal-case">(opsional)</span></label>
                  <textarea
                    rows={2}
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {PALETTES.map((p, i) => (
                      <button
                        ...
                        className={`flex flex-col gap-1.5 p-2 border text-left transition-all ${
                          paletteIdx === i ? "border-neutral-900 bg-white" : "border-neutral-200 hover:border-neutral-400"
                        }`}
                      >
                        ...
                        <span className="flex gap-1">
                          {p.colors.length === 0
                            ...
                            : p.colors.map((c) => (
                              <span key={c} className="w-4 h-4 border border-neutral-200 inline-block" style={{ background: c }} />
                            ))
                          }
                        </span>
                      </button>
                ...
                <Section
                  ...
                  badge={`(${MOCK_COLS.length - 1} dari ${MOCK_COLS.length - 1} dipilih)`}
                  open={colsOpen}
                  onToggle={() => setColsOpen((s) => !s)}
                >
                  <div className="space-y-1">
                    {MOCK_COLS.map((col, i) => (
                      <label key={i} className={`flex items-center gap-2 px-2 py-1.5 text-sm cursor-pointer hover:bg-white transition-colors ${i === 0 ? "opacity-50" : ""}`}>
                        ...
                        <span className="text-neutral-700">{col}{i === 0 && <span className="text-xs text-neutral-400 ml-1">(label)</span>}</span>
                      </label>
                    ))}
                  </div>
                </Section>
                ...
                <Section
                  ...
                  badge={`(${MOCK_ROWS.length} dari 34 dipilih)`}
                  open={rowsOpen}
                  onToggle={() => setRowsOpen((s) => !s)}
                >
                  <div className="relative mb-2">
                    <input
                      type="search"
                      placeholder="Cari baris…"
                      className="w-full pl-3 pr-3 py-1.5 text-xs border border-neutral-300 focus:outline-none focus:border-neutral-900"
                    />
                  </div>
                ...
                <p className="text-xs text-neutral-400">
                  {MOCK_ROWS.length} baris × {MOCK_COLS.length} kolom akan dikirim ke Datawrapper.
                </p>
          ...
          <div className="border border-neutral-200 overflow-hidden">
            ...
            {showRaw && (
              <pre className="bg-neutral-950 text-neutral-300 text-xs p-4 overflow-auto max-h-64 font-mono border-t border-neutral-200">
                {`{\n  "status": "OK",\n  "data-availability": "available",\n  "var": [{"label": "Pengangguran", "val": "2401"}],\n  "datacontent": {\n    "2024": {"11": "6.03", "12": "5.72", "13": "5.44"}\n  }\n}`}
              </pre>
            )}
          </div>
        </div>
